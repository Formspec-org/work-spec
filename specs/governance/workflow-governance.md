...
- **Related-case events.** An `ActivationTrigger` MAY declare `eventScope` (`this` default, or `related`, WOS-INTEG-REL-2101) so a duty, hold resume, or other activation can be matched by an event surfaced from a related case (`$related.*`). Absent means `this`; existing single-case behavior is preserved.
- **DCR constraint zones.** Zone-local flexible-activity sequencing remains DCR (Advanced §4). Cross-state / cross-actor / cross-task durable duties use obligation policies. An optional DCR-response → obligation bridge is an integration, off by default. Per-activity `activationCriteria` gating of activity availability (WOS-INTEG-DCR-1602) is deferred at the schema/typed-model layer: DCR activities are not yet modeled as a typed shape (`advanced.constraintZones` is an untyped array), so there is no per-activity property to attach the criteria to without first introducing that shape.
