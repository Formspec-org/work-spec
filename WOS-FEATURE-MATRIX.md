...
> ^12 **Activation criteria & durable obligations (1.11–1.14):** Introduced by [ADR 0096](thoughts/adr/0096-shared-activation-criteria-and-durable-obligations.md). **Specified + partially implemented (🟦), landed on branch (PR #4):** Governance §16 normative contract; `$defs/ActivationCriteria` / `ObligationPolicy` + `governance.obligationPolicies[]` schema; `wos-core` model + deterministic activation evaluator; `wos-lint` `ACT-001..010` (draft); `wos-events` seven obligation `ProvenanceKind` variants + PROV-O/XES/OCEL export; `wos-runtime` monitor in `drain_once` (activation/satisfy/cancel, pre-event violate-block, lazy deadline expiry, strictest-action gating, replay dedupe, fail-closed); 13 `OBL-001..013` conformance fixtures (**authored, not yet executed** — compilation + exact traces are CI-gated, see WOS-IMPLEMENTATION-STATUS.md §5 and [`docs/obligation-conformance.md`](docs/obligation-conformance.md)). 1.14 (policy-engine materialization) is the optional `obligationHandling: { mode: "materialize" }` path over the implemented §12 bridge. These are **distinct** from DCR constraint zones (1.6, zone-local flexible-activity sequencing), milestones (1.8, data-driven checkpoints), task SLAs (§2.4–2.5), and the deontic `Obligation` (§5, immediate pre-commit agent-output check). FEL gains no temporal operators — it stays the local boolean predicate inside `where`. Competitor marks: ServiceNow/Pega carry SLA/escalation engines that partially approximate durable duties (`~`), but none expose a reusable cross-event obligation primitive.
>
